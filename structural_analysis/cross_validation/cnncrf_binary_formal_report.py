...
def validation_set_factorize(train_X, train_Y, pad_size=880):
    ...
    for i, target in enumerate(train_Y):
        ...
        train_Y_len.append(len(train_Y[i]))
    train_X_augment=[]
    train_Y_augment=[]
    #print(train_Y_new)
    for i, target in enumerate(train_Y_new):
    ...
    train_X_new=torch.stack(train_X_augment)
    train_Y_new=torch.stack(train_Y_augment)
    return train_X_new, train_Y_new, train_Y_len
...
class CrossValidator:
    ...
    def compute(self):
        for i in range(self.partition):
            ...
            self.val_X, self.val_Y, self.val_Y_len=validation_set_factorize(tempval_X, tempval_Y)
            ...
            for j in range(self.epochs):
                if j<9:
                    continue
                print("epoch %i"%j)
                path='/data/yixing/model_cross_checkpoint/cnncrf_binary_archive_validation_check?/cnncrf_binary_train_epoch'+str(j)+'_part'+str(i)+'.pt'
                ...
                for j in range(self.val_X.size(0)):
                    ...
                    last_index=self.val_Y_len[j] #This line might be buggy!!! might want to use the true size
...
out_f=open("cnncrf_binary_validation_report.pkl", "wb")
