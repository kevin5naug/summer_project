...
model.load_state_dict(torch.load('/home/yixing/summer_project/structural_analysis/cross_validation/cnncrf_binary_train_epoch9_part0.pt'))
model.eval()
in_list=[]
target_list=[]
for i in range(VAL_SIZE):
    ...
    temp_in=[]
    temp_target=[]
    X_train = torch.tensor(train_X[i])
    y_train = torch.tensor(train_Y[i])
...
f=open("cnncrf_prediction.pkl", "wb")
print(target_list)
