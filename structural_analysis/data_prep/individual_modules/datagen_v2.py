...
def parse_file(path, output_path):
    ...
    for line in content3:
        ...
        if normalize_t2==-1:
            normalize_t2=begin_t
        if(end_t<ds3.max_t):
            arg_begin_t=float(format(begin_t-normalize_t2+normalize_t1, '.3f'))
            arg_end_t=float(format(end_t-normalize_t2+normalize_t1, '.3f'))
            label, char=ds3.match(arg_begin_t, arg_end_t, path)
            data_third_pass.append([arg_begin_t, arg_end_t, pitch, char, label])
    #Output data file
...
class DataSolverPass3():
    ...
    def match(self, target_begin, target_end, path):
        ...
        while(current_end<=target_begin):
            ...
            current_begin=current_word[2]
            current_end=current_word[3]
        target_diff=float(format(target_end-target_begin, '.3f'))
        current_diff=float(format(current_end-current_begin, '.3f'))
        if((target_begin<current_begin) and (target_end<=current_begin)): #this note locates 1) before a sentence begins; or 2)in the middle of two characters
        ...
        elif(((target_begin==current_begin) and (target_end==current_end))): #this note matches the charactor
            self.state=current_word[0]
            self.current_char=current_word[1]
            self.used_word_flag[self.pointer]=1
            return self.state, self.current_char
        elif(target_end>current_begin):
            if self.used_word_flag[self.pointer]:
                self.current_char="~"
                if self.state==1:
                    self.state=2
                elif self.state==3:
            ...
            print(target_diff, current_diff)
            print(path)
