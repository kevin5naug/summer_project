...
class DataSolverPass3():
    def __init__(self, path):
        ...
        self.normalize_t=-1
        self.pointer=0
        self.char_begin_count=0
        self.char_end_count=0
        self.begin_count=0
        self.end_count=0
    ...
    def match(self, target_begin, target_end, target_char, target_label):
        if target_label==2:
            self.char_begin_count+=1
        elif target_label==4:
            self.char_end_count+=1
        #if target_label in [1,2,4]:
        ...
        for index in range(start_range, len(self.note_sequence)):
            ...
            if(getOverlap(current_note_begin, current_note_end, target_begin, target_end)>0):
                if ((self.label_sequence[index]==-1) and (self.char_sequence[index]=="")):
                    ...
                    print("WARNING: this note has already been assigned a label/char")
                    self.label_sequence[index]=target_label
                    self.char_sequence[index]=target_char
                    self.pointer=index+1
                    return
        if target_label in [2,4]:
            print("WARNING: NOTE CHAR NOT 1-1", target_label, target_char)
    def mark_the_rest(self):
        ...
        if(self.char_begin_count!=self.char_end_count):
            print("ERROR: CHAR BEGIN/END MISMATCHED", self.begin_count, self.end_count, self.path)
        if(self.begin_count!=self.end_count):
            print("ERROR: NOTE BEGIN/END MISMATCHED", self.begin_count, self.end_count, self.path)
        return
    def label_transform(self):
        ...
        for index in range(len(self.note_sequence)):
            ...
            if label==0:
                ...
                count=0
                count+=1
                self.label_sequence[index]=count
            elif label==3:
                count+=1
                self.label_sequence[index]=count
            elif label==4:
                ...
                print("ERROR: LABEL RANGE OUT OF BOUND")
        return
